...
#   - t_pk fijo se estima desde A_perturbation_raw y phi_perturbation_raw del mismo HDF5.
# --------------------------------------------------------------------------------------------------
...
#   - t_pk NO se optimiza.
#   - t_pk se calcula una sola vez desde las perturbaciones crudas Whittaker/arPLS usando el score combinado normalizado.
...
# --------------------------------------------------------------------------------------------------
# t_pk fijo estimado desde los datos Whittaker/arPLS, usando el criterio histórico del notebook.
# --------------------------------------------------------------------------------------------------
...
println("\n================================================================================")
println("t_pk fijo estimado desde perturbaciones crudas Whittaker/arPLS")
println("================================================================================")
println("  k_pk                  = ", k_pk)
...
function save_results_h5_fixed_tpk(filename_hdf5;
    ...
    h5open(filename_hdf5, "w") do f
        ...
        HDF5.attributes(gf)["t_pk_method"] = "argmax sqrt((A_perturbation_raw/std)^2 + (phi_perturbation_raw/std)^2) on zdt_test from Whittaker/arPLS separation H5"
...
filename_hdf5 = "new-perturbation-inversion_F3_nexp_whittakerBG12976567_FIXED_TPK_rel10_results_$(jobid).h5"
...
save_results_h5_fixed_tpk(
    ...
    m_seed = m_seed,
    m_reco = m_reco,
    reco_model = "F3_gaussiana_asimetrica_nexp_vertical_FIXED_TPK_FROM_WHITTAKER_DATA",
...
println("Inversión terminada: F3+nexp con t_pk fijo desde separación Whittaker/arPLS y 6 parámetros libres.")
