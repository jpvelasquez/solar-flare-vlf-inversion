...
# FIRI coefficients are kept as the reference model already defined above.
# For this restart run, the optimizer seed and the search bounds are taken
# from the previous BBO reconstruction, not from FIRI.
const PREVIOUS_RESULTS_H5 = "new-real-background-inversion_from_sep_16_22_results_12909279.h5"
const INITIAL_SOURCE = "previous_BBO_reconstruction_12909279"
const BOUNDS_SOURCE = "±15% around previous BBO reconstruction"

pars_firi0 = vcat(pars_list[1].*1.0, pars_list[2].*1.0)

prev_h_coeffs = Float64.(vec(h5read(PREVIOUS_RESULTS_H5, "reconstruction/h_coeffs")))
prev_beta_coeffs = Float64.(vec(h5read(PREVIOUS_RESULTS_H5, "reconstruction/beta_coeffs")))

if length(prev_h_coeffs) != length(pars_list[1])
    error("Previous h_coeffs length does not match current h coefficients length.")
end
if length(prev_beta_coeffs) != length(pars_list[2])
    error("Previous beta_coeffs length does not match current beta coefficients length.")
end

initial_params = INITIAL_SCALE .* vcat(prev_h_coeffs, prev_beta_coeffs)

lower = SEARCH_LOW_FACTOR .* initial_params
upper = SEARCH_HIGH_FACTOR .* initial_params

println("Restart seed source: ", INITIAL_SOURCE)
println("Previous results HDF5: ", PREVIOUS_RESULTS_H5)
println("Search bounds source: ", BOUNDS_SOURCE)
println("Search factors: [", SEARCH_LOW_FACTOR, ", ", SEARCH_HIGH_FACTOR, "] around restart seed")
...
#################################################################################################
# Evaluar semilla inicial antes de BBO
#################################################################################################
...
println("chi2(initial seed) = ", seed_loss)
...
#################################################################################################
# Guardado limpio: semilla inicial + reconstrucción BBO
#################################################################################################
...
function save_real_background_inversion_h5(
    ...
    h5open(output_h5, "w") do f
        ...
        HDF5.attributes(f)["initial_source"] = String(initial_source)
        HDF5.attributes(f)["bounds_source"] = String(bounds_source)
        ...
        HDF5.attributes(gs)["description"] = "Forward model evaluated at the initial seed coefficients for this run."
...
filename_hdf5 = "new-real-background-inversion_from_sep_16_22_restart15_results_$(jobid).h5"
...
save_real_background_inversion_h5(
    ...
    seed_h_coeffs = initial_params[1:nh],
    seed_beta_coeffs = initial_params[nh+1:end],
    ...
    previous_results_h5 = PREVIOUS_RESULTS_H5,
    target_source = "A_background, phi_background",
)
