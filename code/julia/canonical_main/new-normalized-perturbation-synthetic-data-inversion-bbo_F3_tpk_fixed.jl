...
close(jldfile_ref)
##############################################################################################################################
########################################## Cargando los datos sintéticos #####################################################
##############################################################################################################################
using HDF5
using Dates
using TimeZones
filename_syn = "best_background_perturbation_separation_rmsebg.h5"

# 1) Lee directamente cada arreglo con h5read
A_background_syn      = h5read(filename_syn, "A_background")
A_perturbation_syn    = h5read(filename_syn, "A_perturbation")
phi_background_syn    = h5read(filename_syn, "phi_background")
phi_perturbation_syn  = h5read(filename_syn, "phi_perturbation")
time_unix             = h5read(filename_syn, "timestamps")
...
#################################################################################################################################
baseline_amp   = A_background_syn
baseline_phi   = phi_background_syn
amp_detrended  = A_perturbation_syn
phi_detrended  = phi_perturbation_syn
...
function vlfsignal_perturbation_f3(zdt_test_ref, ϕ_firi_wrapped_ref, mat_sza, pars_list,
    ...
    @assert length(p) == 6 "F3+nexp requiere 6 parámetros: [amp, τ_u, τ_d, eps_t, z0, hz]"
...
### Background cargado desde la PRIMERA inversión del background sintético
background_h5_file = "new-normalized-synthetic-inversion_results_12260814.h5"
@assert isfile(background_h5_file) "No existe el archivo H5 del background sintético: $(background_h5_file)"

bg_res = load_results_h5(background_h5_file)
@assert !isempty(bg_res.reco_params) "El archivo H5 del background no contiene reco_params"
@assert length(bg_res.true_h) > 0 "El archivo H5 del background no contiene true_h"
@assert length(bg_res.true_beta) > 0 "El archivo H5 del background no contiene true_beta"

nh_bg = length(bg_res.true_h)
nβ_bg = length(bg_res.true_beta)
bg_reco_params = collect(Float64.(bg_res.reco_params))

@assert length(bg_reco_params) == nh_bg + nβ_bg "Longitud inesperada en reco_params del background"

coeffs_h_alb = bg_reco_params[1:nh_bg]
coeffs_β_alb = bg_reco_params[nh_bg+1:end]
...
println("Background sintético cargado desde: ", background_h5_file)
...
target_A   = baseline_amp[indices] .+ amp_detrended[indices]
target_phi = baseline_phi[indices] .+ phi_detrended[indices]

# ============================================================================
# t_pk FIJO estimado desde los datos (sobre la misma malla temporal usada en la inversión)
# ============================================================================
...
t_pk_fixed = Dates.value(DateTime(zdt_test[k_pk]) - DateTime(t₀)) / 1000.0  # segundos

println("t_pk fijo desde datos = ", t_pk_fixed, " s respecto a t₀")
println("Instante del pico usado = ", zdt_test[k_pk])
...
target_A_corr = target_A
target_phi_corr = target_phi
...
# LOSS para F3
#################################################################################################
@inline function loss_f3(p, pars_list, mat_sza, target_A, target_phi, t_pk_fixed::Real, cache::SimCache)
    ...
    Aₜ, ϕₜ = try
        vlfsignal_perturbation_f3(zdt_ref, phi_firi_wrapped_ref, mat_sza, pars_list, p, t_pk_fixed, cache)
    catch
        return 1e99
    end
    ...
    println("Norm. error_A: ", error_A/scale_A, ", nor. error_ϕ: ", error_phi/scale_phi)

    χ2 = error_A / scale_A + error_phi / scale_phi
    return isfinite(χ2) ? χ2 : 1e99
end

loss_wrapper(p) = loss_f3(p, pars_list, mat_sza, target_A, target_phi, t_pk_fixed, CACHE)

#####################################################################################################
# Parámetros TRUE de la perturbación usados para generar los sintéticos
# NOTA:
# Estos valores se toman como referencia TRUE del caso sintético.
# La semilla del optimizador se fija en 0.95 .* true_params para no arrancar ya en el mínimo.
#####################################################################################################
const MAXTIME_SEC = 10.0 * 60 * 60

amp_true = 20.0
τ_u_true = 0.052938374 * 3600.0
τ_d_true = 0.34682182  * 3600.0
eps_true = 0.21935698  * 3600.0
z0_true  = 85.0
hz_true  = 10.0

true_params = [amp_true, τ_u_true, τ_d_true, eps_true, z0_true, hz_true]

initial_params = 0.98 .* true_params
lower = 0.95 .* initial_params
upper = 1.05 .* initial_params

println("true_params perturbación = ", true_params)
println("initial_params perturbación = ", initial_params)
...
println("t_pk fijo [s]: ", t_pk_fixed)
println("Parámetros óptimos F3+nexp [amp, τ_u, τ_d, eps, z0, hz]: ", reco_params)

A_reco, ϕ_reco = vlfsignal_perturbation_f3(zdt_ref, phi_firi_wrapped_ref, mat_sza, pars_list, reco_params, t_pk_fixed, CACHE)

jobid = get(ENV, "SLURM_JOB_ID", "nojid")
filename_hdf5 = "new-normalized-perturbation-synthetic-inversion_results_$(jobid).h5"
