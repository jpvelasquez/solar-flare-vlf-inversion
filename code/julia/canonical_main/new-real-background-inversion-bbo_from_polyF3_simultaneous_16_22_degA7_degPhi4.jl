...
#################################################################################################
# Evaluar semilla FIRI después de BBO
#################################################################################################
...
println("chi2(seed FIRI, evaluated after BBO) = ", seed_loss)
...
function save_real_background_inversion_h5(
    ...
    target_source = "PolyF3 simultaneous: A_background, phi_background",
)
    h5open(output_h5, "w") do f
        # ----------------------------------------------------
        # Global metadata
        # ----------------------------------------------------
        HDF5.attributes(f)["method"] = "real_background_inversion_from_polyF3_simultaneous_h5"
        ...
        HDF5.attributes(gs)["description"] = "Forward model evaluated at the FIRI seed coefficients after BBO completion."
...
filename_hdf5 = "new-real-background-inversion_from_polyF3_simultaneous_16_22_degA7_degPhi4_results_$(jobid).h5"
