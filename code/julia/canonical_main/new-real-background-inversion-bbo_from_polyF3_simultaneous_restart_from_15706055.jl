...
##############################################################################################################################
#################### Cargando HDF5 PolyF3 simultáneo seleccionado para inversión de background ################################
##############################################################################################################################

# Archivo generado por el notebook de sensibilidad PolyF3 simultáneo.
...
filename_sep = "simultaneous_polyF3_NAA_V_20080325_window16_22_degA7_degPhi4.h5"

# Lectura directa: este script usa SOLO el esquema del archivo PolyF3 simultáneo seleccionado.
...
A_perturbation_sep = Float64.(vec(h5read(filename_sep, "A_perturbation_F3")))
phi_perturbation_sep = Float64.(vec(h5read(filename_sep, "phi_perturbation_F3")))
...
arrays_to_check = Dict(
    ...
    "A_perturbation_F3" => A_perturbation_sep,
    "phi_perturbation_F3" => phi_perturbation_sep,
...
println("Archivo de separación PolyF3 simultáneo cargado: ", filename_sep)
...
println("  A_perturbation_F3")
println("  phi_perturbation_F3")
println("  A_total_fit")
...
##############################################################
### Restart PolyF3-BBO-2: semilla = best de BBO-1
##############################################################
# Esta segunda corrida NO arranca desde FIRI.
# Arranca desde la reconstrucción best guardada por la primera inversión PolyF3-BBO-1.
# Archivo esperado en el directorio de trabajo.
filename_restart = get(ENV, "RESTART_H5", "new-real-background-inversion_from_polyF3_simultaneous_16_22_degA7_degPhi4_results_15706055.h5")

if !isfile(filename_restart)
    error("No encuentro el HDF5 de restart: " * filename_restart)
end

coeffs_h_restart = Float64.(vec(h5read(filename_restart, "reconstruction/h_coeffs")))
coeffs_β_restart = Float64.(vec(h5read(filename_restart, "reconstruction/beta_coeffs")))
params_restart = Float64.(vec(h5read(filename_restart, "reconstruction/params_vector")))

nh = length(coeffs_h_restart)
nβ = length(coeffs_β_restart)

if length(params_restart) != nh + nβ
    error("Longitud inconsistente en params_vector del restart: length(params_restart)=$(length(params_restart)), nh+nβ=$(nh+nβ)")
end

pars_list = [coeffs_h_restart, coeffs_β_restart]

println("Archivo de restart BBO-1 cargado: ", filename_restart)
println("Seed restart = reconstruction del BBO-1")
println("nh = ", nh, ", nβ = ", nβ)
println("h_coeffs restart = ", coeffs_h_restart)
println("β_coeffs restart = ", coeffs_β_restart)
...
# Restart dirigido alrededor del best de PolyF3-BBO-1.
# Caja: ±5 %.
const SEARCH_LOW_FACTOR = 0.95
const SEARCH_HIGH_FACTOR = 1.05
...
println("Parámetros iniciales restart BBO-1: ", initial_params)
...
#################################################################################################
# Evaluar semilla restart BBO-1 después de BBO
#################################################################################################
...
println("chi2(seed restart BBO-1, evaluated after BBO) = ", seed_loss)
...
#################################################################################################
# Guardado limpio: semilla restart BBO-1 + reconstrucción BBO-2
#################################################################################################
...
function save_real_background_inversion_h5(
    ...
    target_source = "PolyF3 simultaneous: A_background, phi_background; restart seed from PolyF3-BBO-1 job 15706055",
)
    h5open(output_h5, "w") do f
        # ----------------------------------------------------
        # Global metadata
        # ----------------------------------------------------
        HDF5.attributes(f)["method"] = "real_background_inversion_from_polyF3_simultaneous_restart_from_15706055"
        HDF5.attributes(f)["source_separation_h5"] = String(source_separation_h5)
        HDF5.attributes(f)["restart_source_h5"] = String(filename_restart)
        HDF5.attributes(f)["restart_source"] = "PolyF3-BBO-1 best reconstruction, job 15706055"
        ...
        # ----------------------------------------------------
        # Restart seed: best de PolyF3-BBO-1
        # ----------------------------------------------------
        ...
        HDF5.attributes(gs)["description"] = "Forward model evaluated at the PolyF3-BBO-1 best reconstruction used as restart seed, after BBO-2 completion."
        HDF5.attributes(gs)["seed_source"] = "PolyF3-BBO-1 best reconstruction, job 15706055"
...
filename_hdf5 = "new-real-background-inversion_from_polyF3_simultaneous_restart_from_15706055_results_$(jobid).h5"
