...
# Verificación explícita: el archivo ASLS empieza antes (16:00),
...
#################################################################################################
const AMP_OFFSET_DB_USED = 21.0#19.0 + 1.48
const PHI_OFFSET_DEG_USED = -190.0#-210.0
...
println("weighted chi2(seed FIRI) = ", seed_loss)
println("unweighted chi2(seed FIRI) = ", seed_components.χ2_unweighted)
...
function save_real_background_inversion_h5(
    ...
    h5open(output_h5, "w") do f
        ...
        HDF5.attributes(f)["initial_source"] = "FIRI_seed"
        HDF5.attributes(f)["bounds_source"] = "search_low_factor/search_high_factor around FIRI seed"
...
filename_hdf5 = "new-real-background-inversion-bbo_from_old_asls_results_$(jobid).h5"
