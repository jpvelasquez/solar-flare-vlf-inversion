...
dt_utc     = epoch .+ Second.(round.(Int, time_unix))   # Vector{DateTime} en UTC
tz_utc     = tz"UTC"
...
##############################################################
### Se tomaron los polinomios de grado 5 para h y 8 para beta.
###############################################################
...
ts_data = timestamps_test#[...]  # tiempo en formato UNIX
target_A = baseline_amp[indices]
target_phi = baseline_phi[indices]
##################################################
...
##############################################
# 2) LOSS normalizada y robusta para background
##############################################
@inline function loss_bg(pp_flat, mat_sza, target_A, target_phi, cache::SimCache)
...
loss_wrapper(pp) = loss_bg(pp, mat_sza, target_A, target_phi, CACHE)
const MAXTIME_SEC = 10.0 * 60 * 60#1*20*60#10 * 60 * 60 #- 5*60#1 * 10 * 60  #10 minutos /1/2 hora

pars0 = vcat(pars_list[1].*1.0, pars_list[2].*1.0)
initial_params = 0.98 .* pars0
...
result = BlackBoxOptim.bboptimize(loss_wrapper, initial_params;
                 Method          = :dxnes,#:adaptive_de_rand_1_bin_radiuslimited,#dxnes
                 SearchRange     = search_range,
                 NumDimensions   = num_params,
                 PopulationSize  = 20,
                 MaxFuncEvals    = 1e9,#2,#300,      # o menos si LMP es muy lento
                 TraceMode       = :compact,
                 MaxTime         = MAXTIME_SEC,
                 NThreads        = nthreads)    # quita spam en consola
...
A_reco, ϕ_reco = vlfsignal_simple_perturbation_v2(zdt_ref, phi_firi_wrapped_ref, mat_sza, pp_reco, CACHE)
jobid = get(ENV, "SLURM_JOB_ID", "nojid")
#println("Julia jobid: ",jobid)
################################################################################
χ2_init = loss_wrapper(initial_params)
χ2_best = best_fitness(result)  # o loss_wrapper(best_candidate(result)) si quieres re-evaluar
println("chi2(init) = ", χ2_init)
println("chi2(best) = ", χ2_best)
################################################################################
filename_hdf5 = "new-normalized-synthetic-inversion_results_$(jobid).h5"
parameter_names = vcat(["h_c$(i-1)" for i in 1:length(pars_list[1])], ["beta_c$(i-1)" for i in 1:length(pars_list[2])])
