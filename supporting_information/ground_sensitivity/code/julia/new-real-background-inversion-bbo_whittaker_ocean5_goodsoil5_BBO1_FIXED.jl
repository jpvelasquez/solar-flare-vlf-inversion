...
################################################################################
# Whittaker-background inversion: heterogeneous lower boundary
#
# Ground configuration, ordered from NAA (TX) to PLO (RX):
#   [GROUND[10] × 5, GROUND[7] × 5]
#
# This is an independent BBO-1 inversion:
#   - same Whittaker target used by the homogeneous-seawater inversion;
#   - same hard-coded FIRI seed;
#   - same ±8% search range and normalized objective;
#   - same 10-waveguide geometry and 18:00–22:00 UTC sampling;
#   - ground and FIRI phase-reference file changed consistently to the
#     ocean5_goodsoil5 configuration.
################################################################################
...
println("="^88)
println("Whittaker-background heterogeneous inversion — BBO-1")
...
filename_hdf5 = joinpath(
    ...
    "new-real-background-inversion_whittaker_ocean5_goodsoil5_BBO1_results_$(jobid).h5",
)
